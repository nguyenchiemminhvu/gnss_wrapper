...
void ubx_database_wrapper::update_datum(const ubx::parser::datum_data& data)
{
    {
        std::lock_guard<std::mutex> lk(datum_mutex_);
        datum_ = data;
    }
    data_updated.emit(gnss::buffer_type::datum_config);
}

const ubx::parser::datum_data& ubx_database_wrapper::get_datum() const
{
    std::lock_guard<std::mutex> lk(datum_mutex_);
    return datum_;
}

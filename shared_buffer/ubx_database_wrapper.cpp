...
const attitude_data& ubx_database_wrapper::get_attitude() const
{
    std::lock_guard<std::mutex> lk(att_mutex_);
    return attitude_;
}

const raw_measurements_data& ubx_database_wrapper::get_raw_measurements() const
{
    std::lock_guard<std::mutex> lk(raw_meas_mutex_);
    return raw_measurements_;
}

const monitor_data& ubx_database_wrapper::get_monitor() const
{
    std::lock_guard<std::mutex> lk(mon_mutex_);
    return monitor_;
}

const security_data& ubx_database_wrapper::get_security() const
{
...
void ubx_database_wrapper::on_lp_commit(const ubx::database::ubx_location_snapshot& snap)
{
    ...
    // ── Monitor buffer (UBX-MON-IO + UBX-MON-TXBUF) ──────────────────────────
    {
        ...
        if (mon.io_valid || mon.txbuf_valid)
        {
            mon.valid = true;
            {
                std::lock_guard<std::mutex> lk(mon_mutex_);

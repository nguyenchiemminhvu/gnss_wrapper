...
public:
    ...
    const gnss::dead_reckoning_data_out&    get_dead_reckoning_out() const override;
    const gnss::dead_reckoning_data_in&     get_dead_reckoning_in()  const override;

    /// Update the datum buffer (called from gnss_controller datum_resolved callback).
    void update_dead_reckoning_in(const gnss::dead_reckoning_data_in& data) override;
...
private:
    ...
    mutable std::mutex dr_out_mutex_;
    mutable std::mutex dr_in_mutex_;
    ...
    gnss::dead_reckoning_data_out    dead_reckoning_out_{};
    gnss::dead_reckoning_data_in     dead_reckoning_in_{}; // not exposed via getter, only updated by service
};

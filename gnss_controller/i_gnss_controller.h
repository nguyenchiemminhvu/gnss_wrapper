...
public:
    ...
    /// Perform a hardware reset of the GNSS chip by pulsing the RESET_N GPIO
    /// low for at least 100 ms (per ZED-F9P integration manual §3.9.2).
    ///
    /// The method blocks for the pulse duration then returns.  Receivers are
    /// NOT automatically restarted; the caller must issue start() again if
    /// needed.
    ///
    /// Returns false (no-op) if no RESET_N GPIO pin has been configured via
    /// set_reset_pin().  Returns false on any GPIO I/O failure.
    virtual bool hardware_reset() = 0;

    /// Inject the RESET_N GPIO pin to be used by hardware_reset().
    /// Must be called before hardware_reset() is first invoked.
    /// Passing nullptr disables the hardware-reset capability.
    /// @param pin  A concrete i_gpio_pin implementation for the RESET_N line.
    virtual void set_reset_pin(std::shared_ptr<gnss::i_gpio_pin> pin) = 0;
    ...
    /// Force re-application of the default config on failed sync attempt.
    virtual bool force_sync_config(const std::string& ini_path) = 0;

    ...
    , config_repo_(std::make_shared<ubx::config::ubx_config_repository>())
{
    ...
    {
        LOG_INFO("[gnss_controller] configuring UART on ", cfg.device_path.c_str());
        (void)cfg;
...
bool gnss_controller::init()
{
            ...
            registry.register_decoder(
                std::make_unique<ubx::parser::mon_ver_decoder>(
                    [this](const ubx::parser::ubx_mon_ver& decoded) {
                        LOG_INFO("[gnss_controller] received MON-VER response: ", decoded.sw_version, " / ", decoded.hw_version);
            ...
            registry.register_decoder(
                std::make_unique<ubx::parser::cfg_valget_decoder>(
                    [this](const ubx::parser::ubx_cfg_valget& decoded) {
                        ...
                        {
                            LOG_INFO("[gnss_controller] configuration sync completed successfully; default config applied to chip");
    ...
    for (std::size_t i = 0; i < receivers_.size(); ++i)
    {
        ...
        {
            LOG_ERROR("[gnss_controller] receiver[", i, "] init failed on '", configs_[i].device_path, "'");
            all_ok = false;
        }
        else
        {
...
bool gnss_controller::start()
{
    ...
    for (std::size_t i = 0; i < receivers_.size(); ++i)
    {
        ...
        {
            LOG_ERROR("[gnss_controller] receiver[", i, "] start failed");
            all_ok = false;
        }
        else
        {

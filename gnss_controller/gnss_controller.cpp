            ...
            registry.register_decoder(
                std::make_unique<ubx::parser::upd_sos_decoder>(
                    [this](const ubx::parser::ubx_upd_sos_output& decoded) {
                        if (decoded.cmd == ubx::parser::UPD_SOS_CMD_ACK) // backup operation acknowledgement
                        {
                            transport_->handle_ack(ubx::parser::UBX_CLASS_UPD, ubx::parser::UBX_ID_UPD_SOS, true);
                            if (decoded.response == ubx::parser::UPD_SOS_BACKUP_RESP_ACK)
                            {
                                LOG_INFO("[gnss_controller] UPD-SOS backup operation acknowledged by receiver");
                                ubx_database_->data_updated.emit(gnss::buffer_type::backup_operation_completed);
                            }
                            else
                            {
                                LOG_INFO("[gnss_controller] UPD-SOS backup operation not acknowledged by receiver");
                            }
                        }
                        else if (decoded.cmd == ubx::parser::UPD_SOS_CMD_RESTORE) // restored from backup acknowledgement
                        {
                            transport_->handle_ack(ubx::parser::UBX_CLASS_UPD, ubx::parser::UBX_ID_UPD_SOS, true);
                            if (decoded.response == ubx::parser::UPD_SOS_RESTORE_RESP_FAILED)
                            {
                                LOG_INFO("[gnss_controller] UPD-SOS restore operation failed on receiver");
                            }
                            else if (decoded.response == ubx::parser::UPD_SOS_RESTORE_RESP_SUCCESS)
                            {
                                LOG_INFO("[gnss_controller] UPD-SOS restore operation acknowledged by receiver");
                                ubx_database_->data_updated.emit(gnss::buffer_type::restore_operation_completed);
                            }
                            else if (decoded.response == ubx::parser::UPD_SOS_RESTORE_RESP_NO_BACKUP)
                            {
                                LOG_INFO("[gnss_controller] UPD-SOS restore operation failed: no backup present on receiver");
                            }
                            else
                            {
                                LOG_INFO("[gnss_controller] UPD-SOS restore operation not acknowledged by receiver");
                            }
                        }
                    }));
        });
...
bool gnss_controller::force_sync_config(const std::string& ini_path)
{
    LOG_INFO("[gnss_controller] force applying default config sync");

    std::lock_guard<std::mutex> lk(cmd_mutex_);

    if (!config_manager_->load_default_config(ini_path))
    {
        LOG_ERROR("[gnss_controller] force sync failed to load default config from INI file: ", ini_path);
        return false;
    }

    if (!config_manager_->force_apply_pending_sync())
    {
        LOG_ERROR("[gnss_controller] force sync failed to apply default config");
        return false;
    }

...
public:
    ...
    // ── i_gnss_controller ─────────────────────────────────────────────────────
    bool hardware_reset()                         override;
    void set_reset_pin(std::shared_ptr<gnss::i_gpio_pin> pin) override;
    ...
    bool force_sync_config(const std::string& ini_path) override;
...
private:
    // ── Private slot functions for external sensor data update ────────────────
    void slot_external_data_update(gnss::buffer_type type);
...
private:
    ...
    // GPIO pin for hardware reset — optional; null means hardware_reset() is a no-op.
    std::shared_ptr<gnss::i_gpio_pin> reset_n_pin_;
};

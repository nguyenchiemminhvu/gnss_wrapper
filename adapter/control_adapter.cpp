...
#include "commands/force_sync_config_command.h"
...
#include "commands/hardware_reset_command.h"
...
void control_adapter::set_reset_pin(std::shared_ptr<gnss::i_gpio_pin> pin)
{
    controller_->set_reset_pin(std::move(pin));
}
...
void control_adapter::build_command_table()
{
    ...
    // "hardware_reset" — pulse RESET_N GPIO low for ≥ 100 ms (ZED-F9P §3.9.2).
    // Requires set_reset_pin() to have been called with a valid i_gpio_pin.
    command_table_[gnss::CMD_HARDWARE_RESET] = [this](const gnss::parsed_command&) -> std::unique_ptr<gnss::i_gnss_command> {
        return std::make_unique<gnss::hardware_reset_command>(controller_);
    };
    ...
    command_table_[gnss::CMD_FORCE_SYNC_CONFIG] = [this](const gnss::parsed_command& p) -> std::unique_ptr<gnss::i_gnss_command> {
        std::string path = p.args.empty() ? DEFAULT_CONFIG_INI_PATH : p.args[0];
        return std::make_unique<gnss::force_sync_config_command>(controller_, std::move(path));
    };

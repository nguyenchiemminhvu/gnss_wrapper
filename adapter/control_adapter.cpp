...
void control_adapter::build_command_table()
{
    ...
    command_table_[gnss::CMD_SYNC_CONFIG] = [this](const gnss::parsed_command& p) -> std::unique_ptr<gnss::i_gnss_command> {
        std::string path = p.args.empty() ? DEFAULT_CONFIG_INI_PATH : p.args[0];
        return std::make_unique<gnss::sync_config_command>(controller_, std::move(path));
    };
}

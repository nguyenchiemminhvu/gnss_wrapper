...
enum class buffer_type : uint8_t
{
    ...
    // ── Extended UBX message buffers ─────────────────────────────────────────
    attitude         = 6u, ///< attitude_data_buffer was refreshed (UBX-NAV-ATT)
    raw_measurements = 7u, ///< raw_measurements_data_buffer was refreshed (UBX-RXM-MEASX)
    monitor          = 8u, ///< monitor_data_buffer was refreshed (UBX-MON-IO, UBX-MON-TXBUF)
    security         = 9u, ///< security_data_buffer was refreshed (UBX-SEC-SIG)
};
...
static constexpr int UART_MAX_RECONFIGURE_RETRIES = 5;

...
enum class buffer_type : uint8_t
{
    ...
    dead_reckoning_out          , ///< dead_reckoning_data_buffer was refreshed (UBX-ESF-INS + UBX-ESF-STATUS)
    dead_reckoning_in           , ///< dead_reckoning_data_buffer was refreshed with input sensor measurements (UBX-ESF-MEAS)
    ...
    restore_operation_completed , ///< restore operation (restore from backup) acknowledged by chip
...
/// Default filesystem paths for UBX record/replay files used in testing and diagnostics.
extern const char* DEFAULT_UBX_RECORD_PATH;

/// Default filesystem path for the INI configuration file.
extern const char* DEFAULT_CONFIG_INI_PATH;

/// Default sysfs filesystem path for the GNSS chip RESET_N GPIO value file.
/// BSP layers typically export this as /sys/class/gpio/gpioN/value or a
/// custom virtual file that accepts "0"/"1" writes.
extern const char* DEFAULT_RESET_N_GPIO_PATH;
...
extern const char* CMD_HARDWARE_RESET;
...
extern const char* CMD_FORCE_SYNC_CONFIG;

/// Datum configuration commands.
extern const char* CMD_QUERY_DATUM;
extern const char* CMD_SET_DATUM;
extern const char* CMD_SET_DATUM_WGS84;
extern const char* CMD_SET_DATUM_PZ90;
extern const char* CMD_RESET_DATUM;

/// Record / replay commands.
/// "start_record"                       — begin recording raw UBX frames to file.
/// "stop_record"                        — stop recording.
/// "start_replay"                       — switch all receivers to replay from file.
/// "stop_replay"                        — restore all receivers to live I/O input.
extern const char* CMD_START_RECORD;
extern const char* CMD_STOP_RECORD;
extern const char* CMD_START_REPLAY;
extern const char* CMD_STOP_REPLAY;
...
/// Sleep duration when no data available, to avoid busy-spinning in the worker loop
/// when POLLIN is signaled but available() returns 0 (e.g. non-looping file port at EOF).
static constexpr int32_t UART_NO_DATA_SPIN_LOOP_SLEEP_MS = 50;

/// Poll duration is counted from the time a poll command is sent until receiving ACK message.
static constexpr uint16_t UART_POLL_TIMEOUT_MS = 3000U;
...
/// Minimum duration the RESET_N GPIO must be held LOW to guarantee a cold-start
/// on the ZED-F9P (integration manual §3.9.2 specifies ≥ 100 ms).  A small
/// safety margin is added so the constant is 150 ms.
static constexpr uint32_t GNSS_RESET_N_PULSE_DURATION_MS = 150U;

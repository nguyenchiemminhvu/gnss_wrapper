 ...
 *
 * Four independent sections, each wrapped in its own function so you can call
 * the one that matches your integration point:
 *
 ...
 *   3. section_receiver()      — low-level ubx_gnss_receiver + ubx_database_wrapper wiring
...
#include "gnss_receiver/ubx_gnss_receiver.h"      // UART worker thread + parser
...
// signals emitted by base_database / ubx_database_wrapper.
...
public:
    ...
    // Slot connected to base_database::data_updated signal.
    // Called from the ubx_gnss_receiver worker thread — keep it fast.
...
static void section_adapter()
{
    ...
    adapter.execute_command("sync_config:" + std::string(gnss::DEFAULT_CONFIG_INI_PATH));
...
static void section_controller()
{
    ...
    auto db  = ctrl.get_database();
    auto mgr = std::make_unique<LocationManagerSubscriber>(db);
    db->data_updated.connect(mgr.get(), &LocationManagerSubscriber::on_data_updated);

    // Sync configuration from INI before issuing any resets.
    if (!ctrl.sync_config(gnss::DEFAULT_CONFIG_INI_PATH))
        std::puts("[WARN] sync_config returned false — INI may be missing");
...
// ==========================================================================
// SECTION 3 — ubx_gnss_receiver (UART worker thread + parser, lowest level)
//
// Normally you won't construct ubx_gnss_receiver directly — gnss_controller owns
...
{
    std::puts("\n=== SECTION 3: ubx_gnss_receiver (direct) ===\n");
    ...
    // 2. Construct the receiver and attach its database so that setup()
    //    can register the UBX decoder stubs with the parser.
    auto receiver = std::make_unique<gnss::ubx_gnss_receiver>(port);
    receiver->setup(database);                 // registers NAV-PVT, NAV-SAT, etc.
...
static void section_database_only()
{
    ...
    // (e.g. obtained from adapter.get_database() or ctrl.get_database()).
